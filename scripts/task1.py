...
# Team Robot: Octorok
# Group Member Name and ID: Lonnie Chien lchien1
# Group Member Name and ID: Suraj Kalidindi skalidi1
# Group Member Name and ID: Toby Le tle1
...
class Controller:
    ...
    def __init__(self):
        ...
        self.MAX_LINEAR_CHANGE = 0.001 # TODO: set to reasonable value
	...
	if abs(beta) <= (math.pi/180):
	    self.toDrive = True
	cmd_vel.angular.z = 0.1*beta + 0.5*np.sign(beta)
        cmd_vel.linear.x = 0
	...
	beta = angle_from_vec(point)
        # TODO: task 1 - make the robot turn towards the specified point
	cmd_vel.linear.x = 0.25*point[0] + 0.1
	if point[0] > 0.05:
		cmd_vel.angular.z = 0.2*beta
	else:
	        cmd_vel.angular.z = 0
        return cmd_vel
	...
	cur_pose = self.get_current_pose()
	    ...
	    self.dest = T_robot_from_world*self.start_pose*[2,1]
	    if self.toDrive == False: 
               vel = self.turn_towards(self.dest)
	       cmd_vel = vel
            else: 
               vel = self.drive_towards(self.dest)
               cmd_vel = vel
		      
            
            if cmd_vel.linear.x == 0 and self.toDrive:
