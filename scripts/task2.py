...
class Controller:
    ...
    def __init__(self):
        ...
        self.MAX_LINEAR_CHANGE = 0.006 # TODO: set to reasonable value
        
        # maximum change in angular velocity per 0.01s timestep
        self.MAX_ANGULAR_CHANGE = 0.01 # TODO: set to reasonable value
	#Task 1 variables
    ...
    def turn_to_ball(self, point):
        ...
        if abs(beta) <= (math.pi/180 + 0.15): 
            self.to_kick = True
        cmd_vel.angular.z = 0.5*beta + 0.5*np.sign(beta)
        cmd_vel.linear.x = 0
        return cmd_vel
    ...
    def hit_ball(self, point):	
    	cmd_vel = Twist() 
        beta = angle_from_vec(point)
        cmd_vel.linear.x = np.abs(2*point[0] + 2)
        if point[0] > 0.1: 
            cmd_vel.angular.z = 0.05*beta 
        else: 
            cmd_vel.angular.z = 0
        #    cmd_vel.linear.x = 0
	return cmd_vel
            ...
            if self.ball_in_world_frame is None: 
                ...
                if self.goal_in_world_frame is not None and self.ball_in_world_frame is not None:
           	    if self.target_position is None:
                        #goal_to_ball_vec = self.ball_in_world_frame - self.goal_in_world_frame
			dx = self.ball_in_world_frame.x - self.goal_in_world_frame.x
			dy = self.ball_in_world_frame.y - self.goal_in_world_frame.y
			#dtheta = self.ball_in_world_frame.theta - self.goal_in_world_frame.theta
                        goal_to_ball_vec = [dx, dy]
                        c = (dx*dx + dy*dy)**0.5
			c_prime = c + 0.8 
                        factor = c_prime/c
                        dx_prime = dx*factor 
                        dy_prime = dy*factor  
                        target_x = self.goal_in_world_frame.x + dx_prime 
                        target_y = self.goal_in_world_frame.y + dy_prime 
